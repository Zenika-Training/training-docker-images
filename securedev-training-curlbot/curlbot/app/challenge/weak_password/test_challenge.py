...
def setup_function():
    ...
    with app.app_context():
        ...
        db.drop_all()
        db.create_all()
        from app.challenge.weak_password import seed
        seed(app)
    return app
...
class TestChallengeClass:    
  ...
  def test_challenge_is_exploitatble(self, client):
    ...
    assert client.get("/api/v1/flag").status_code == 401
    assert self.post_json(client,"/api/v1/auth/login",{"login": "admin", "password": "matrix"}).json['status']=="success"
